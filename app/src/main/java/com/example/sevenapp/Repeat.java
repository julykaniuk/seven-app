...
public class Repeat extends AppCompatActivity implements DatePickerDialog.OnDateSetListener{
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                if (idIndex != -1) {
                    ...
                    // Handle the case where COLUMN_ID is not found
                }
    ...
    public void hasOldRepeat() {
        ...
        Cursor cursor = mDatabase.rawQuery(SQLQuery, null);
        if (cursor.moveToFirst()) {
            // Check if COLUMN_SERI and COLUMN_SERI_TYPE exist
            ...
            if (seriIndex != -1 && seriTypeIndex != -1) {
                do {
                    int seriValue = cursor.getInt(seriIndex);
                    if (seriValue > -1) { // Check if seriValue is not -1
                ...
                // Handle the case where either COLUMN_SERI or COLUMN_SERI_TYPE is not found
            }
        }
    }
